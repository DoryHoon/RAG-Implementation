# Multi-Query RAG — 삼성전자 4Q25 실적 분석 (HuggingFace 버전)

단일 쿼리 RAG의 벡터 유사도 한계를 극복하기 위해 Multi-Query 기법을 적용한 실험 프로젝트. 삼성전자 2025년 4분기 실적 발표 자료(PDF 2종)를 소스 문서로 사용했으며, 전 과정을 HuggingFace 오픈소스 모델로 구성했다.
...
- **소스 문서 조정:** 프레젠테이션 PDF 제거, 오디오 스크립트만 사용 — 자연어 문장 형태로 숫자와 라벨이 같은 문장 안에 존재하여 파싱 오류 최소화
- **벡터 스토어:** `FAISS` → `Chroma`
